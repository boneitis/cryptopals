...
def step_2c_calc_ri_start(b, s_i_last, n):
  numerator = (b * s_i_last) - (2 * B)
  return ceil(2 * (numerator / n))
...
#      print('CHECKPOINT 2 ; si:[', sa, ',', sb, ']') ####
      ...
      print('  s_i', s_i) ####
      m_i = I.empty()
      ra = s3_calc_ra(M_last[0], s_i, n)
      rb = s3_calc_rb(M_last[1], s_i, n)

      ri = ra
#      print('CHECKPOINT 3 ; r:[', ra, ',', rb, ']', end=' ')
      print('    r:[', ra, ',', rb, ']', end=' ')
      while True:
        print(ri, end=' ') ####
        aa = s3_calc_aa(ri, n, s_i)
        bb = s3_calc_bb(ri, n, s_i)
        aaa = max(M_last[0], aa)
        bbb = min(M_last[1], bb)
        m_i |= I.closed(aaa, bbb)
...
#      print('CHECKPOINT 5') ####
      ...
      if M_last[0] == M_last[1]:
        break
      s_i_last = s_i # expected to reach here only if a conformant s_i was found
#      print('CHECKPOINT 6') ####
...
#    print((M_last[0]).to_bytes(96, 'big'), end='\n\n')
    print(M_last[0])
    print(M_last[1], end='\n\n')
